...
function initParm(){
	var parmsModel;
	if(jsonStr!=''){
		initSearchParm(jsonStr,false);
	}else{
		initSearchParm('<%=uid%>',true);	
	}
}

function initSearchParm(jsonStr,isuid){
	var surl='../../initPreParm.action';
	var sdata={};
	if(isuid){
		url+='?uid='+jsonStr;
	}else{
		sdata={reportJson:JSON.stringify(jsonStr)}
	}
	$.ajax({
            url : surl, 
            cache:false,
